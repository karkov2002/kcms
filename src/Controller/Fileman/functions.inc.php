...
{
    $ret = false;
    if (0 === mb_strpos($path.'/', getFilesPath())) {
        $ret = true;
    }
...
function verifyAction($action)
{
    if (!defined($action) || !constant($action)) {
        ...
        $basePath = str_replace('/fileman', '/', BASE_PATH);
        $confUrl = $basePath.'/'.$confUrl;
...
function getFilesNumber($path, $type)
{
    ...
    foreach ($tmp as $ff) {
        if ('.' == $ff || '..' == $ff) {
            continue;
        } elseif (is_file($path.'/'.$ff) && ('' == $type || ('image' == $type && RoxyFile::IsImage($ff)) || ('flash' == $type && RoxyFile::IsFlash($ff)))) {
            ++$files;
...
class RoxyFile
{
    ...
    public static function IsImage($fileName)
    {
        ...
        if ('jpg' == $ext || 'jpeg' == $ext || 'jpe' == $ext || 'png' == $ext || 'gif' == $ext || 'ico' == $ext) {
            $ret = true;
        }
    ...
    public static function IsFlash($fileName)
    {
        $ret = false;
        $ext = strtolower(self::GetExtension($fileName));
        if ('swf' == $ext || 'flv' == $ext || 'swc' == $ext || 'swt' == $ext) {
            $ret = true;
        }
